...
- 当时模型供应商和向量维度尚未确定，数据库设计不能提前锁死具体模型。
...
- 在模型确定前，`knowledge_chunk.embedding` 使用不固定维度的 `vector` 字段，并暂缓创建 HNSW 索引。该阶段性决定已由 [ADR-0002](ADR-0002-bailian-embedding-and-hybrid-retrieval.md) 的 1024 维方案接续。
